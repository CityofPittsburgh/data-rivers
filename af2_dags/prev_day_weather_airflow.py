...
def init_cmds_xcomm(**kwargs):
    ...
    kwargs['ti'].xcom_push(key="prev_day_weather_gsc", value=f"python {os.environ['GCS_LOADER_PATH']}"
                 "/prev_day_weather_gcs.py --lookback_date")
    ...
    kwargs['ti'].xcom_push(key="beam_cleanup", value=airflow_utils.beam_cleanup_statement(bucket))
...
prev_day_weather_gcs = BashOperator(
    ...
    bash_command=str("{{ ti.xcom_pull(task_ids='push_xcom', key='prev_day_weather_gsc') }}") + " {{ prev_ds }}",
    dag=dag
)
...
prev_day_weather_bq_load = GoogleCloudStorageToBigQueryOperator(
    ...
    source_objects=["prev_day_weather/{{ prev_ds|get_prev_ds_year }}/{{ prev_ds|get_prev_ds_month }}/" +
                    "{{ prev_ds }}_weather_report.avro"],
