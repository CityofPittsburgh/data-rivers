...
def standardize_parc_num(self, datum):
    """
       ...
       each of the 5 CITY_PARC_NUM_FORMAT of the parcel number string are fixed in length (4, 1, 5, 4,
       and 2 characters,
       respectively)
       and no characters can be omitted (e.g. 1234X12345123412 is correct) Zero padding the CITY_PARC_NUM_FORMAT is
       very
       common (
       i.e. each component frequently contains several "0") (0001X00001000101 is correctly formatted).

       The city frequently inserts hyphens between CITY_PARC_NUM_FORMAT and does not zero pad them. As an example,
       this parcel
       number: 0001X00001000101 (when correctly formatted) could appear like this in the city's datasets: 01-X-01-1-1.
       Further, when all CITY_PARC_NUM_FORMAT of the parcel number are "0"s the city frequently omits them. As an
       example:
       01-X-01
    ...
    # skip Nulls
    try:
        parc_str_extract = datum["JOBPARCELXREF"][0]["PARCEL"]["PARCELNUMBER"]
    except:
        ...
        # some city formats contain hyphens and abbreviated components
        ...
        # parts will be a list with all 5 segments of the parc number
        ...
        for i in range(len(parts)):
            ...
            # verify all chars in component are correctly alpha or non alpha CITY_PARC_NUM_FORMAT (as of August 2023,
            # only component #1 and #3 cannot have a letter) (special char containing strings already bypassed-
        ...
        # data for internal testing purposes etc. The minimum of 2  unique characters prevents test strings like "0000A00000000000"
        # from passing through. This is, however, only a weak safeguard. End users should understand that manual
        # inspection may be required depending on the use case.
