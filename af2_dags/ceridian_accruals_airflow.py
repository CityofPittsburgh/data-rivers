...
# The goal of this DAG is to extract Time Bank accruals from all officers present in the Ceridan system for comparison
...
bucket = f"gs://{os.environ['GCS_PREFIX']}_ceridan"
dataset = 'accruals'
...
accruals_gcs_loader = BashOperator(
    ...
    bash_command=F"python {os.environ['DAGS_PATH']}/dependencies/gcs_loaders/ceridan_accruals_gcs.py "
                 F"--output_arg {dataset}/{json_loc}",
    dag=dag
)

exec_df = f"python {os.environ['DATAFLOW_SCRIPT_PATH']}/ceridan_accruals_dataflow.py"
accruals_dataflow = BashOperator(
    task_id='ceridan_accruals_dataflow',
...
accruals_gcs_to_bq = GoogleCloudStorageToBigQueryOperator(
    ...
    destination_project_dataset_table=f"{os.environ['GCLOUD_PROJECT']}:ceridan.time_{output_name}",
...
accruals_beam_cleanup = BashOperator(
    ...
    bash_command=airflow_utils.beam_cleanup_statement(f"{os.environ['GCS_PREFIX']}_ceridan"),
    dag=dag
)

accruals_gcs_loader >> accruals_dataflow >> accruals_gcs_to_bq >> delete_accruals_avro >> \
