...
source_dir = "finance"
bucket = f"gs://{os.environ['GCS_PREFIX']}_{source_dir}"
...
tax_delinquency_gcs = BashOperator(
    ...
    bash_command=f"python {os.environ['GCS_LOADER_PATH']}/{source_dir}_{dataset}_gis_wprdc_extract.py --output_arg {json_loc}",
    dag=dag
)
...
# for each parcel
sub_query = q.build_geo_coords_from_parcel_query(f"{os.environ['GCLOUD_PROJECT']}.{source_dir}.incoming_{table}",
                                                 "parc_num")
coord_query = f"CREATE OR REPLACE TABLE  `{os.environ['GCLOUD_PROJECT']}.{source_dir}.incoming_{table}` AS {sub_query}"
...
CREATE OR REPLACE TABLE `{os.environ['GCLOUD_PROJECT']}.{source_dir}.{dataset}_partitioned` 
...
FROM 
  `{os.environ['GCLOUD_PROJECT']}.{source_dir}.geo_enriched_{table}`;"""
...
query_export_cols = f"""
CREATE OR REPLACE TABLE `{os.environ['GCLOUD_PROJECT']}.{source_dir}.delinquency_export_wprdc` AS
SELECT
    {COLS}
FROM
    `{os.environ['GCLOUD_PROJECT']}.{source_dir}.{dataset}_partitioned`
"""
...
wprdc_export = BigQueryToCloudStorageOperator(
    ...
    source_project_dataset_table=f"{os.environ['GCLOUD_PROJECT']}.{source_dir}.delinquency_export_wprdc",
    destination_cloud_storage_uris=[f"gs://{os.environ['GCS_PREFIX']}_wprdc/{source_dir}/{dataset}/{export_file}.csv"],
    bigquery_conn_id='google_cloud_default',
    dag=dag
)
...
delete_export = BigQueryTableDeleteOperator(
    ...
    deletion_dataset_table=f"{os.environ['GCLOUD_PROJECT']}.{source_dir}.delinquency_export_wprdc",
    bigquery_conn_id='google_cloud_default',
    dag=dag
)
...
query_push_gis = F"""
CREATE OR REPLACE TABLE `data-bridgis.{source_dir}.{dataset}_partitioned` AS
SELECT 
* 
FROM 
  `{os.environ['GCLOUD_PROJECT']}.{source_dir}.delinquency_export_wprdc`;
"""
