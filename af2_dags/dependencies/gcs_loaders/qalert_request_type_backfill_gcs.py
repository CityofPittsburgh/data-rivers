...
# set the limit for how many records can be simultaneously sent to the Google DLP API
API_LIMIT = 2000
# init the backfill constants (last major DAG update was 8/25/22)
DEFAULT_RUN_START = "2022-08-25 00:00:00"
...
run_stop_win = "2022-08-25 00:00:00"
...
while datetime.strptime(run_stop_win, "%Y-%m-%d %H:%M:%S") <= datetime.strptime(BACKFILL_STOP, "%Y-%m-%d %H:%M:%S"):
    ...
    run_start_win, first_run = find_last_successful_run(bucket, "requests/backfill/request_type_backfill/successful_run_log/log.json",
                                                        DEFAULT_RUN_START)
    ...
    json_to_gcs("requests/backfill/request_type_backfill/successful_run_log/log.json", [successful_run],
                bucket)

    append_target_path = f"requests/backfill/request_type_backfill/{BACKFILL_STOP.split(' ')[0]}/backfilled_req_type_requests.json"
    curr_run_target_path = f"requests/backfill/request_type_backfill/{BACKFILL_STOP.split(' ')[0]}/{run_stop_win.split(' ')[0]}.json"
    temp_target_path = f"requests/backfill/request_type_backfill/{BACKFILL_STOP.split(' ')[0]}/temp_uploaded_blob.json"
