...
# TODO: The GCS loader was/can be run outside of the DAG for testing purposes. (Execution time for 04-2015 through
# 11-2021 bacfill was approx 1 hour). Uncomment this if it necessary to re-run.
...
WITH formatted AS
    ...
    FROM 
        {os.environ['GCLOUD_PROJECT']}.qalert.backfill_req
    )
...
qalert_requests_format_dedupe = BigQueryOperator(
        ...
        destination_dataset_table = f"{os.environ['GCLOUD_PROJECT']}:qalert.backfill_req",
        write_disposition = 'WRITE_TRUNCATE',
        dag = dag
)

# Query new tickets to determine if they are in the city limits
query_city_lim = build_city_limits_query('qalert', 'backfill_req', 'pii_lat', 'pii_long')
...
query_append = f"""
INSERT INTO {os.environ['GCLOUD_PROJECT']}.qalert.backfill_all_actions
SELECT 
    {COLS_IN_ORDER} 
FROM 
